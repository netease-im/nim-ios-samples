...
extension V2NIMUserSearchOption {
  ...
  static func fromDic(_ arguments: [String: Any]) -> V2NIMUserSearchOption {
    ...
    if let keyword = arguments["userSearchOption.keyword"] as? String {
      option.keyword = keyword
    }

    if let searchName = arguments["userSearchOption.searchName"] as? Bool {
      option.searchName = searchName
    }

    if let searchAccountId = arguments["userSearchOption.searchAccountId"] as? Bool {
      option.searchAccountId = searchAccountId
    }

    if let searchMobile = arguments["userSearchOption.searchMobile"] as? Bool {
