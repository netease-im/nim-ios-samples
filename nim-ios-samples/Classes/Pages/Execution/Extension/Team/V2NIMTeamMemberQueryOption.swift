...
extension V2NIMTeamMemberQueryOption {
  ...
  static func fromDic(_ arguments: [String: Any]) -> V2NIMTeamMemberQueryOption {
    ...
    if let roleQueryType = arguments["queryOption.roleQueryType"] as? Int,
       let roleQueryTypeValue = V2NIMTeamMemberRoleQueryType(rawValue: roleQueryType) {
      option.roleQueryType = roleQueryTypeValue
    }

    if let onlyChatBanned = arguments["queryOption.onlyChatBanned"] as? Bool {
      option.onlyChatBanned = onlyChatBanned
    }

    if let direction = arguments["queryOption.direction"] as? Int, let directionValue = V2NIMQueryDirection(rawValue: direction) {
      option.direction = directionValue
    }

    if let nextToken = arguments["queryOption.nextToken"] as? String {
      option.nextToken = nextToken
    }

if let limit = arguments["queryOption.limit"] as? Int {
      option.limit = limit
    }
    return option
  }
